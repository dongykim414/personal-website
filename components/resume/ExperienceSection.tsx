...
function TechList({ items, label }: {
    ...
    return (<ul className="tech-list" aria-label={label}>
...
export function DetailGrid({ details }: {
    details: DetailBlock[];
}) {
    return (<div className="detail-grid">
      {details.map((detail) => (<div className="detail-row" key={detail.label}>
          ...
          <div className="detail-content">
            ...
            {detail.result ? (<div className="service-result">
            ...
            {detail.groups ? (<div className="work-groups">
                {detail.groups.map((group) => (<section className="work-group" key={group.title}>
                    <h5>{group.title}</h5>
...
export function ProjectExperience({ project }: {
    project: Project;
}) {
    const { t } = useLanguage();
    return (<article className="project" id={project.id}>
      ...
      <div className="project-content">
        ...
        <h4>
          <a className="project-anchor" href={`#${project.id}`} aria-label={t("{0} 프로젝트 바로가기", project.title)}>
        ...
        <TechList items={project.tech} label={t("{0} 사용 기술", project.title)}/>
        <DetailGrid details={project.details}/>
        {(project.id === "challenge-today" || project.id === "webtoon-metric") && <Link className="resume-case-link" href={`/projects/${project.id}`}>{t("문제 해결 과정과 데모 보기 →")}</Link>}
      </div>
    </article>);
}
export function ProjectTimeline({ projects }: {
    ...
    return (<div className="project-timeline">
      {projects.map((project) => (<ProjectExperience key={project.title} project={project}/>))}
    </div>);
}
